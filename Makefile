...
.clasp.json:
	make login
	clasp create --title slack-esa-unfurling --type webapp --rootDir ./src
	clasp setting fileExtension ts
	# clasp setting filePushOrder
	sed -i -e 's/}/,"filePushOrder":["src\/SlackOAuth2Handler.ts","src\/EsaOAuth2Handler.ts","src\/SlackBaseHandler.ts","src\/BaseError.ts","src\/JobBroker.ts"]}/' .clasp.json
