...
class JobBroker {
  public static readonly MAX_SLOT = 10;
  public static readonly DELAY_DURATION = 150;
  // 1 Hour
  public static readonly JOB_EXECUTE_TIME_OUT = 3600;
  /**
   * 15 min
   * see. https://developers.google.com/apps-script/reference/script/clock-trigger-builder#atdateyear,-month,-day
   */
  public static readonly JOB_STARTING_TIME_OUT = 900;
  ...
  public enqueue(callback: JobFunction, parameter: {}): void {
  ...
  public dequeue(handler: string): Job | null {
    ...
    for (const trigger of this.triggers) {
      ...
      const parameter: JobParameter = this.getJobParameter(trigger);
      if (parameter) {
        if (!this.isExpire(parameter)) {
          if (parameter.state === "waiting" && handler === parameter.handler) {
            if (
              !parameter.hasOwnProperty("scheduled_at") ||
              parameter.scheduled_at <= this.now
            ) {
              return {
                parameter,
                trigger,
  ...
  protected createJob(callback: JobFunction, parameter: {}): Job {
    const trigger = ScriptApp.newTrigger(callback.name)
      .timeBased()
      .after(JobBroker.DELAY_DURATION)
      .create();
  ...
  private getCacheExpirationInSeconds(
    ...
    switch (jobParameter.state) {
      case "waiting":
        if (jobParameter.hasOwnProperty("scheduled_at")) {
          return (
            JobBroker.JOB_STARTING_TIME_OUT +
            Math.round((jobParameter.scheduled_at - this.now) / 1000)
          );
        } else {
          // JOB_STARTING_TIME_OUT + DELAY_DURATION(round up milliseconds)
          return JobBroker.JOB_STARTING_TIME_OUT + 1;
        }
      case "starting":
        return JobBroker.JOB_EXECUTE_TIME_OUT;
      case "end":
        // expire
        return 0;
      case "failed":
  ...
  private isExpire(parameter: JobParameter): boolean {
    switch (parameter.state) {
      case "waiting":
        if (parameter.hasOwnProperty("scheduled_at")) {
          return (
