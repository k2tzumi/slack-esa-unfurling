...
class SlackApiClient {
  ...
  public chatUnfurl(
    ...
    user_auth_required: boolean = false,
    user_auth_url: string = null
  ): void {
    const endPoint = SlackApiClient.BASE_PATH + "chat.unfurl";
    let payload: {} = {
      channel,
      ts,
      unfurls
    ...
    if (user_auth_message) {
      payload = { ...payload, user_auth_message };
    }
    if (user_auth_required) {
      payload = { ...payload, user_auth_required };
    }
    if (user_auth_url) {
      payload = { ...payload, user_auth_url };
    }
